...
public class RockerView extends View {
    ...
    private OnLenXYListener onLenXYListener;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int width = ViewTool.getScreenHeight(getContext());
        measureWidth = width * 2 / 3;
    ...
    private Point getRockerPositionPoint(Point centerPoint, Point touchPoint, float regionRadius, float rockerRadius) {
        ...
        if (lenXY + rockerRadius <= regionRadius) { // 触摸位置在可活动范围内
            if (onLenXYListener != null) {
                onLenXYListener.lenXY(lenXY + rockerRadius, regionRadius);
            }
            return touchPoint;
        } else { // 触摸位置在可活动范围以外
            // 计算要显示的位置
    ...
    public void setOnLenXYListener(OnLenXYListener listener) {
        this.onLenXYListener = listener;
    }
    ...
    public interface OnLenXYListener {

        void lenXY(float lenXY, float radus);
